...
function hexToVec3(hex: string): [number, number, number] {
  const h = hex.replace('#', '');
  return [
    parseInt(h.slice(0, 2) || 'ff', 16) / 255,
    parseInt(h.slice(2, 4) || 'ff', 16) / 255,
    parseInt(h.slice(4, 6) || 'ff', 16) / 255,
  ];
}
...
float auroraGlow(float t, vec2 shift) {
  vec2 uv = gl_FragCoord.xy / max(uResolution.y, 1.0);
  uv += shift;
...
export default function SoftAurora({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('SoftAurora WebGL fallback to CSS glow:', err);
